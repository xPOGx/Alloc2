...
Alloc2::Alloc2(const size_t n, const size_t ps) {
	pages = n / ps;
...
void* Alloc2::alloc_addr(size_t s) {
...
PageDescriptor Alloc2::defineCategory(size_t s) {
	PageDescriptor pd;
	if (s > (pageSize >> 1)) {
		pd.state = 2;
		pd.bsize = s / pageSize;
...
size_t Alloc2::defineBlockSize(size_t s) {
	size_t res = 16;
	while (s > res) {
		res <<= 1;
	}
...
size_t* Alloc2::getFreeMBlock(size_t ps) {
...
bool Alloc2::freeLBlockIsLast(PageDescriptor pd) {
...
size_t* Alloc2::getFreeLBlock(size_t bs) {
...
size_t Alloc2::createLBlockPage(size_t bs) {
...
size_t* Alloc2::getAbsolutePageAddr(size_t index) {
	size_t* res = NULL;
	if (index <= pages) {
...
size_t Alloc2::getFreePage() {
	PageDescriptor pd = pageDescriptors[firstFreePage];
	firstFreePage = pd.next;
	return pages + 1;
}

void Alloc2::setAllFree(PageDescriptor pd) {
	size_t bs = pd.bsize;
...
size_t Alloc2::getIndex(size_t s) {
	size_t counter = 0;
	while (s > 1) {
		counter++;
		s >>= s;
...
void* Alloc2::alloc_ReAddr(void* addr, size_t size) {
	if (addr == NULL) {
		return alloc_addr(size);
	}
	...
	release_addr(addr);
	size_t* newAddr = (size_t*)alloc_addr(size);
	size_t length = min(size, usefulMem);
	copyData(beg, newAddr, length);
	return newAddr;
}

void Alloc2::copyData(size_t* from, size_t* to, size_t length) {
	if (from > to) {
...
void Alloc2::release_addr(void* addr) {
...
size_t Alloc2::findPageByAddress(size_t* addr) {
	size_t shiftFromBegin = addr - pagesBegin;
	size_t pageNumber = shiftFromBegin / pageSize;
	return pageNumber;
}

size_t Alloc2::findBlockByAddress(size_t* addr, size_t bs) {
	size_t shiftFromBegin = addr - pagesBegin;
